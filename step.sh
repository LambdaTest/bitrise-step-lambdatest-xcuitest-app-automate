...
upload_app_response="$(curl --location --request POST https://$lambdatest_username:$lambdatest_access_key@stage-app-api.lambdatest.com/app/uploadFramework --form type="xcui-ios" --form appFile=@$app_ipa_path)"
app_url=$(echo "$upload_app_response" | jq .app_id)

echo "uploading test ipa/zip to LambdaTest"
upload_test_response="$(curl --location --request POST https://$lambdatest_username:$lambdatest_access_key@stage-app-api.lambdatest.com/app/uploadFramework --form type="xcui-ios" --form appFile=@$test_ipa_path)"
...
run_test_response="$(curl --location --request POST https://qa-beta-api.lambdatest.com/framework/v1/build --header "Authorization: Basic ${BASICAUTH}" --header "Content-Type: application/json" --data-raw \ "$json")"
